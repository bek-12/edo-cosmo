...
interface SummaryTopProduct { productName: string; unitsSold: number; revenue: number; }
...
function exportCSV(summary: SummaryData) {
  const rows = [
    ["Period", summary.period], ["Total Sales", summary.totalSales],
    ["Total Revenue", summary.totalRevenue], ["Total Spent", summary.totalSpent],
    ["Net Profit", summary.netProfit], ["Return Rate", `${summary.returnRate}%`],
...
function exportPurchaseCSV(report: PurchaseReport) {
  const rows = [
    ["Period", report.period], ["Total Spent", report.totalSpent],
    ["Purchases Made", report.totalPurchases], ["Units Restocked", report.totalUnitsRestocked],
...
async function exportPurchasePDF(report: PurchaseReport) {
  ...
  let y = 18;
  doc.setFillColor(...INDIGO); doc.rect(0, 0, 210, 14, "F");
  ...
  autoTable(doc, {
    ...
    body: report.recentPurchases.map((p) => [
      ...
      p.productName, p.category, String(p.quantity), fmtBirr(p.buyingPrice), fmtBirr(p.totalCost),
    ]),
...
function exportEarningsCSV(report: EarningsReport) {
  const rows = [
...
async function exportEarningsPDF(report: EarningsReport) {
  const doc = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" });
  ...
  doc.text(`Generated: ${now}`, 14, y); y += 10;
  const kpis = [
    { label: "Total Gain",    value: fmtBirr(report.totalGain) },
    { label: "Total Revenue", value: fmtBirr(report.totalRevenue) },
  ...
  autoTable(doc, {
    ...
    body: report.topProfitableProducts.map((p, i) => [String(i+1), p.productName, String(p.unitsSold), fmtBirr(p.totalGain), `${p.marginPercent}%`]),
    theme: "striped", styles: { fontSize: 9, cellPadding: 5 },
    headStyles: { fillColor: GREEN, textColor: [255,255,255] as [number,number,number], fontStyle: "bold" },
    columnStyles: { 0: { cellWidth: 12, halign: "center" }, 1: { cellWidth: 80 }, 2: { cellWidth: 20, halign: "center" }, 3: { cellWidth: 40, halign: "right" }, 4: { cellWidth: 30, halign: "right" } },
    margin: { left: 14, right: 14 },
  });
...
async function exportPDF(summary: SummaryData) {
  ...
  let y = 18;
  doc.setFillColor(...ROSE); doc.rect(0, 0, 210, 14, "F");
  doc.setTextColor(255, 255, 255); doc.setFontSize(13); doc.setFont("helvetica", "bold");
  doc.text("Cosmetics Shop — Sales Report", 14, 9.5);
  doc.setTextColor(...DARK); doc.setFontSize(16); doc.setFont("helvetica", "bold");
  doc.text(`${periodLabel} Summary`, 14, y + 6); y += 8;
  doc.setFontSize(9); doc.setFont("helvetica", "normal"); doc.setTextColor(...GRAY);
  doc.text(`Generated: ${now}`, 14, y + 4); y += 12;
  const kpis = [
  ...
  kpis.forEach((kpi, i) => {
    const x = startX + i * (boxW + gap);
    doc.setFillColor(...LIGHT); doc.roundedRect(x, y, boxW, boxH, 2, 2, "F");
    doc.setDrawColor(229, 231, 235); doc.roundedRect(x, y, boxW, boxH, 2, 2, "S");
    doc.setFontSize(7); doc.setFont("helvetica", "normal"); doc.setTextColor(...GRAY);
    doc.text(kpi.label.toUpperCase(), x + 3, y + 5);
    doc.setFontSize(10); doc.setFont("helvetica", "bold"); doc.setTextColor(...kpi.color);
    doc.text(kpi.value, x + 3, y + 13);
  });
  y += boxH + 10;
  doc.setFontSize(11); doc.setFont("helvetica", "bold"); doc.setTextColor(...DARK);
  doc.text("Top Products", 14, y); y += 4;
  autoTable(doc, {
    startY: y, head: [["#", "Product", "Units Sold", "Revenue"]],
    body: summary.topProducts.map((p, i) => [String(i+1), p.productName, String(p.unitsSold), fmtBirr(p.revenue)]),
    theme: "striped", styles: { fontSize: 10, cellPadding: 6, overflow: "linebreak" },
    headStyles: { fillColor: ROSE, textColor: [255,255,255] as [number,number,number], fontStyle: "bold", halign: "left" },
    tableWidth: 182,
    columnStyles: {
      0: { cellWidth: 20, halign: "center" }, 1: { cellWidth: 92, halign: "left" },
...
/* ── Stat cards — period received from parent ── */
function SummarySection({ period, onSummaryReady }: {
  period: "weekly" | "monthly" | "yearly";
  onSummaryReady: (s: SummaryData | null) => void;
}) {
...
function EarningsSection({ period, onReportReady }: {
  period: "weekly" | "monthly" | "yearly";
  onReportReady: (r: EarningsReport | null) => void;
}) {
  ...
  return (
    <div className="mb-4 space-y-4">
      {loading ? (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm flex items-center justify-center h-32">
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {[
              ...
              { label: "Average Margin",  value: `${report.averageMarginPercent}%`,    sub: "gain ÷ revenue",     accent: "text-emerald-600" },
            ].map((kpi, i) => (
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="p-4 border-b border-gray-100">
              <h3 className="text-xs font-semibold text-gray-600 uppercase tracking-wide mb-3">Most Profitable Products</h3>
...
export default function ReportsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"sales" | "purchases" | "earnings">("sales");
  ...
  const [currentEarningsReport, setCurrentEarningsReport] = useState<EarningsReport | null>(null);
  ...
  const openReturn = async (sale: Sale) => {
    ...
    try {
      ...
      setReturnItems(sale.items.map((si) => ({
        productId: si.productId, quantity: 0,
        maxQty: si.quantity - (alreadyReturned[si.productId] ?? 0),
        priceAtSale: si.priceAtSale, productName: si.product.name,
      })));
      ...
      <div className="p-4 sm:p-6 max-w-7xl mx-auto">
        ...
        <div className="flex items-start justify-between mb-4 sm:mb-6">
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            {activeTab === "earnings" && currentEarningsReport && (
              <>
                <button onClick={() => exportEarningsCSV(currentEarningsReport)}
                  className="flex items-center gap-1.5 px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-sm font-medium transition-colors">
                  <Download className="w-4 h-4" />Earnings CSV
                </button>
                <button onClick={() => exportEarningsPDF(currentEarningsReport)}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-5">
          <div className="flex flex-wrap gap-1 bg-gray-100 p-1 rounded-xl w-fit">
            ...
            <button onClick={() => { setActiveTab("earnings"); setSalesSearch(""); setPurchasesSearch(""); }}
              className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeTab === "earnings" ? "bg-white shadow text-emerald-700" : "text-gray-500 hover:text-gray-700"}`}>
              My Earnings
            </button>
          </div>
            ...
            {(["weekly", "monthly", "yearly"] as const).map((p) => (
              <button key={p} onClick={() => setPeriod(p)}
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
                  period === p
                    ? activeTab === "purchases" ? "bg-indigo-500 text-white"
                      : activeTab === "earnings" ? "bg-emerald-500 text-white"
        ...
        {activeTab === "earnings"  && <EarningsSection  period={period} onReportReady={setCurrentEarningsReport} />}
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-100">
                  <tr className="text-left text-gray-500">
                    ...
                    <th className="px-4 py-3 font-medium text-right hidden sm:table-cell">Unit Price</th>
