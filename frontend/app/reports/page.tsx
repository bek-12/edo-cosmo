...
/* â”€â”€ Earnings / Profit Report Types â”€â”€ */
/* â”€â”€ Helpers â”€â”€ */
const fmt = (n: number) =>
...
async function exportPurchasePDF(report: PurchaseReport) {
  ...
  doc.text("Cosmetics Shop â€” Purchases Report", 14, 9.5);
...
async function exportPDF(summary: SummaryData) {
  ...
  doc.setTextColor(255, 255, 255); doc.setFontSize(13); doc.setFont("helvetica", "bold");
  doc.text("Cosmetics Shop â€” Sales Report", 14, 9.5);
  doc.setTextColor(...DARK); doc.setFontSize(16); doc.setFont("helvetica", "bold");
  ...
  const kpis = [
    ...
    { label: summary.isLoss ? "Net Loss" : "Net Profit",
      value: `${summary.isLoss ? "âˆ’" : "+"}${fmtBirr(Math.abs(summary.netProfit))}`,
  ...
  doc.text(`Return Rate: ${summary.returnRate}%   Â·   Cosmetics Shop Sales & Inventory System`, 14, finalY);
  doc.save(`report-${summary.period}-${new Date().toISOString().split("T")[0]}.pdf`);
}

/* â”€â”€ Stat cards â€” period received from parent â”€â”€ */
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { label: summary.isLoss ? "Net Loss" : "Net Profit",
              value: `${summary.isLoss ? "âˆ’" : "+"}${fmt(Math.abs(summary.netProfit))}`,
              sub: `Return rate ${summary.returnRate}% Â· sales minus restocking`,
  ...
  return (
    <Layout>
      <div className="p-4 sm:p-6 max-w-7xl mx-auto">
            ...
            {(["weekly", "monthly", "yearly"] as const).map((p) => (
              <button key={p} onClick={() => setPeriod(p)}
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
                  period === p
                    ? activeTab === "purchases" ? "bg-indigo-500 text-white" : "bg-rose-500 text-white"
        ...
        {activeTab === "sales" && (
          <>
            {/* Filters + Search â€” one row on desktop, stacked on mobile */}
            <div className="mb-5">
          ...
          <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-lg max-h-[90vh] overflow-y-auto animate-slide-up sm:animate-none">
            ...
            <div className="px-5 py-4 space-y-4">
              ...
              <p className="text-xs text-gray-500">
                ...
                {" Â· "}<span className="font-medium text-gray-700">{fmt(returnSale.totalAmount)}</span>
              </p>
                ...
                {returnItems.map((ri) => (
                  <div key={ri.productId} className={`flex items-center justify-between p-3 rounded-lg border ${ri.quantity > 0 ? "border-rose-200 bg-rose-50" : "border-gray-100 bg-gray-50"}`}>
                    <div>
                      ...
                      <p className="text-xs text-gray-400">{fmt(ri.priceAtSale)} each Â· max return {ri.maxQty}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <button type="button" onClick={() => updateReturnQty(ri.productId, Math.max(0, ri.quantity - 1))}
                        disabled={ri.quantity === 0}
                        className="w-7 h-7 bg-gray-200 hover:bg-rose-100 rounded-full flex items-center justify-center disabled:opacity-40 font-bold text-lg leading-none">âˆ’</button>
